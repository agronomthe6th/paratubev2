  ...
  extraReducers: {
    [FetchVideoRating.fulfilled]: (state, { payload }) => {
      try {
        // console.log('payload');
        // payload.results[0] ????
        // console.log('RefreshRatings succ', payload[0]);
        state.likesCount = payload[0].likes_count;
        state.dislikesCount = payload[0].dislikes_count;
        state.isLiking = payload[0].is_liking;
        state.isDisLiking = payload[0].is_disliking;
        state.Ready = true;
      } catch (e) {
        console.log('RefreshRatings error', payload[0]);
      }
